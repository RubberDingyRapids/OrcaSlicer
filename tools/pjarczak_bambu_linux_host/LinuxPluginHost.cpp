...
void translate_print_params_paths(Slic3r::PrintParams& p)
{
...
Slic3r::TaskQueryParams task_query_from_json(const nlohmann::json& j)
{
    Slic3r::TaskQueryParams p{};
...
Slic3r::PrintParams print_params_from_json(const nlohmann::json& j)
{
...
nlohmann::json LinuxPluginHost::handle(const std::string& method, const nlohmann::json& payload)
{
    ...
    if (method == "net.bind") {
        // 02.08.01 series: bind() takes dev_model after dev_id.
    ...
    if (method == "net.start_print") {
        auto f = net<int (*)(void*, Slic3r::PrintParams, OnUpdateStatusFn, WasCancelledFn, OnWaitFn)>("bambu_network_start_print");
        auto a = lookup_agent();
        if (!f || !a) return not_supported(method);
        const auto job_id = payload.value("client_job_id", 0LL);
    ...
    if (method == "net.start_local_print_with_record") {
        auto f = net<int (*)(void*, Slic3r::PrintParams, OnUpdateStatusFn, WasCancelledFn, OnWaitFn)>("bambu_network_start_local_print_with_record");
        auto a = lookup_agent();
        if (!f || !a) return not_supported(method);
        const auto job_id = payload.value("client_job_id", 0LL);
        const auto params_json = payload.value("params", nlohmann::json::object());
    ...
    if (method == "net.start_local_print") {
        auto f = net<int (*)(void*, Slic3r::PrintParams, OnUpdateStatusFn, WasCancelledFn)>("bambu_network_start_local_print");
        auto a = lookup_agent();
        if (!f || !a) return not_supported(method);
        const auto job_id = payload.value("client_job_id", 0LL);
        const auto params_json = payload.value("params", nlohmann::json::object());
    ...
    if (method == "net.start_send_gcode_to_sdcard") {
        auto f = net<int (*)(void*, Slic3r::PrintParams, OnUpdateStatusFn, WasCancelledFn, OnWaitFn)>("bambu_network_start_send_gcode_to_sdcard");
        auto a = lookup_agent();
        if (!f || !a) return not_supported(method);
        const auto job_id = payload.value("client_job_id", 0LL);
        const auto params_json = payload.value("params", nlohmann::json::object());
    ...
    if (method == "net.start_sdcard_print") {
        auto f = net<int (*)(void*, Slic3r::PrintParams, OnUpdateStatusFn, WasCancelledFn)>("bambu_network_start_sdcard_print");
        auto a = lookup_agent();
        if (!f || !a) return not_supported(method);
        const auto job_id = payload.value("client_job_id", 0LL);
        const auto params_json = payload.value("params", nlohmann::json::object());
    ...
    // 02.08.01 series: filament spool book-keeping and AMS sync (all plain request/reply).
    if (method == "net.get_filament_spools") { auto f = net<int (*)(void*, FilamentQueryParams, std::string*)>("bambu_network_get_filament_spools"); auto a = lookup_agent(); if (!f || !a) return not_supported(method); std::string http_body; const int ret = f(a, JsonBridge::filament_query_params_from_json(payload.value("params", nlohmann::json::object())), &http_body); return {{"ok", true}, {"value", ret}, {"http_body", http_body}}; }
    ...
    if (method == "net.sync_ams_filaments") { auto f = net<int (*)(void*, AmsSyncParams, std::string*)>("bambu_network_sync_ams_filaments"); auto a = lookup_agent(); if (!f || !a) return not_supported(method); std::string http_body; const int ret = f(a, JsonBridge::ams_sync_params_from_json(payload.value("params", nlohmann::json::object())), &http_body); return {{"ok", true}, {"value", ret}, {"http_body", http_body}}; }
