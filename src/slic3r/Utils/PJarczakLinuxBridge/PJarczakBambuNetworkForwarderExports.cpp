...
// func_get_hms_snapshot takes the strings by reference; the export must match or the callee
// would destroy the caller's objects.
...
// 02.08.01 series: filament spool book-keeping and AMS sync.
PJBRIDGE_EXPORT int bambu_network_get_filament_spools(void* agent, FilamentQueryParams params, std::string* http_body) { auto* a = require_agent(agent); if (!a) return invalid_handle(); const auto j = ok_or_error(RpcClient::instance().invoke_json("net.get_filament_spools", {{"agent", agent_id(a)}, {"params", JsonBridge::to_json(params)}})); if (http_body) *http_body = j.value("http_body", std::string()); return j.value("value", 0); }
PJBRIDGE_EXPORT int bambu_network_create_filament_spool(void* agent, std::string request_body, std::string* http_body) { auto* a = require_agent(agent); if (!a) return invalid_handle(); const auto j = ok_or_error(RpcClient::instance().invoke_json("net.create_filament_spool", {{"agent", agent_id(a)}, {"request_body", request_body}})); if (http_body) *http_body = j.value("http_body", std::string()); return j.value("value", 0); }
PJBRIDGE_EXPORT int bambu_network_update_filament_spool(void* agent, std::string spool_id, std::string request_body, std::string* http_body) { auto* a = require_agent(agent); if (!a) return invalid_handle(); const auto j = ok_or_error(RpcClient::instance().invoke_json("net.update_filament_spool", {{"agent", agent_id(a)}, {"spool_id", spool_id}, {"request_body", request_body}})); if (http_body) *http_body = j.value("http_body", std::string()); return j.value("value", 0); }
PJBRIDGE_EXPORT int bambu_network_delete_filament_spools(void* agent, FilamentDeleteParams params, std::string* http_body) { auto* a = require_agent(agent); if (!a) return invalid_handle(); const auto j = ok_or_error(RpcClient::instance().invoke_json("net.delete_filament_spools", {{"agent", agent_id(a)}, {"params", JsonBridge::to_json(params)}})); if (http_body) *http_body = j.value("http_body", std::string()); return j.value("value", 0); }
PJBRIDGE_EXPORT int bambu_network_get_filament_config(void* agent, std::string* http_body) { auto* a = require_agent(agent); if (!a) return invalid_handle(); const auto j = ok_or_error(RpcClient::instance().invoke_json("net.get_filament_config", {{"agent", agent_id(a)}})); if (http_body) *http_body = j.value("http_body", std::string()); return j.value("value", 0); }
PJBRIDGE_EXPORT int bambu_network_sync_ams_filaments(void* agent, AmsSyncParams params, std::string* http_body) { auto* a = require_agent(agent); if (!a) return invalid_handle(); const auto j = ok_or_error(RpcClient::instance().invoke_json("net.sync_ams_filaments", {{"agent", agent_id(a)}, {"params", JsonBridge::to_json(params)}})); if (http_body) *http_body = j.value("http_body", std::string()); return j.value("value", 0); }
